...
def respace_dp(dictionary: List[str], sentence: str) -> int:
    ...
    # v1 版本的问题在于:
    问题出在，你每次遇见比如字母i，然后词典里i开头的有一些词，你不知道要选哪个，甚至可以说，你不知道要不要选
    假如句子是iamyourbrother，词典里就两个词：iamyour和amyourbrother
    遇到i的时候，甚至是不能选的
    每次不在于“当前位置往后匹配啥最好”

    DP的核心是，不管之后的事, DP是这样的，和数列一样，需要一个递归公式
    即，已知在我之前，有了最优解如此，那么有了我，最优解会如何变化, 至于我之后，最优解怎么样，不管
    类似数学归纳法, 假设N-1你已经解出来了，然后把N往N-1上靠，能靠上，就可以dp
    """
    pass
